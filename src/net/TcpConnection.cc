...
void TcpConnection::send1(Buffer *buf)
{
    // 当属于正在连接的状态
    if (state_ == kConnected)
    {
        ...
        {
            loop_->runInLoop(std::bind(&TcpConnection::sendInLoop1, this, buf->retrieveAllAsString()));
        }
    }
}

void TcpConnection::sendInLoop1(const std::string& message)
{
